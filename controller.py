...
VIRTUAL_GATEWAY = {
    "192.168.103.2": "00:00:00:00:00:01",
    "192.168.2.2": "00:00:00:00:00:02",
    "192.168.3.2": "00:00:00:00:00:03",
    "192.168.4.2": "00:00:00:00:00:04",
}
BROADCAST_TTL = 1
...
class TopoAwareness(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.broadcast_packet = {}  # 用于存储广播数据包的时间戳

# -----------------------------------------NEW FUNCTION----------------------------------------- #
    def _record_broadcast(self, src, dst):
        key = (src, dst)
        if key not in self.broadcast_packet:
            self.broadcast_packet[key] = time.time()

    def _check_broadcast_suppression(self, src, dst):
        ...
        if key in self.broadcast_packet:
            ...
            if BROADCAST_TTL < elapsed:
                ...
                for datapath in datapaths:
                    ...
                    def add_flow(datapath, priority, match, actions, idle_timeout, buffer_id=None):
                        ofproto = datapath.ofproto
                        parser = datapath.ofproto_parser
                    ...
                    add_flow(datapath, priority=10, match=match, actions=actions, idle_timeout=int(BROADCAST_TTL / 2))
...
# 
    ...
    def _switch_packet_in_handle(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP and pkt.opcode == arp.ARP_REQUEST:
            self.logger.info("ARP请求包: 源IP=%s, 目标IP=%s, 源MAC=%s, 目标MAC=%s, 交换机=%s, 端口=%s",  
                                    src_ip, dst_ip, src_mac, dst_mac, dpid, in_port)
            # 抑制ARP广播风暴
            if self._check_broadcast_suppression(src_ip, dst_ip):
                return
            # 构造ARP回复
            if dst_ip in VIRTUAL_GATEWAY.keys():
                self._send_arp_reply(datapath, in_port, src_mac, src_ip, VIRTUAL_GATEWAY[dst_ip], dst_ip)
                return
            for (dpid, src_mac_arp, dst_ip_arp) in self.arp_table:
                if dst_ip_arp == dst_ip:
                    # 发送ARP回复
                    self._send_arp_reply(datapath, in_port, src_mac, src_ip, src_mac_arp, dst_ip_arp)
                    return
        # 过滤非ARP的虚拟网关数据包
        if dst_ip in VIRTUAL_GATEWAY.keys():
            return
        ...
        if dst_mac in self.mac_to_port[dpid]:  # 这里的mac_to_port与851行中的mac_to_port存放的MAC地址和port的映射不一样
            ...
            self._record_broadcast(src_ip, dst_ip)
            if eth.ethertype == ether_types.ETH_TYPE_ARP:
                self.logger.info("*************************ARP**************************************************")
...
#_lldp_packet_in_handle 和 _host_arp_packet_in_handle 方法：分别处理LLDP和ARP数据包，用于发现链路和主机信息。
    ...
    def _host_arp_packet_in_handle(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP or eth.ethertype == ether_types.ETH_TYPE_IP:
            ...
            #     self.logger.info("1433主机学习禁止，交换机无端口, 源IP %s", src_ip)
            #     return

            # 关键：检查是否是交换机间的链路端口
            if self.is_link_port(dpid, in_port):
                # 只在非链路端口学习主机
                self.logger.info("1433主机学习禁止，非链路端口, src_ip=%s", src_ip)
                return
            
            for link in self.topo_access_link.keys():
                if dpid == link[0] and in_port == self.topo_access_link[link][0]:
                    self.logger.info("1433主机学习禁止 src ip=%s", src_ip)
                # 更新时间戳信息
